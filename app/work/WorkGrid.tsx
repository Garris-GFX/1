...
function pickFeaturedThree(items: WorkMeta[]) {
  const tagged = items.filter((p) =>
    p.tags?.some((t) => normalize(t) === "featured")
  );
  const base = tagged.length ? tagged : items;
  return base.slice(0, 3);
}

function getPillsForWork(p: WorkMeta) {
  // prefer services, fallback to tags
  const out: string[] = [];
  if (p.services?.length) out.push(...p.services.map(String));
  else if (p.tags?.length) out.push(...p.tags.map(String));
  return out;
}

function workMatchesPill(p: WorkMeta, pill: string) {
  const target = normalize(pill);
  const services = p.services?.map((s) => normalize(s)) ?? [];
  const tags = p.tags?.map((t) => normalize(t)) ?? [];
  return services.length ? services.includes(target) : tags.includes(target);
}

function workSearchText(p: WorkMeta) {
  return [p.title, p.excerpt, ...(p.tags ?? []), ...(p.services ?? []), p.date]
    .filter(Boolean)
    .join(" ");
}

export default function WorkGrid({ projects }: { projects: WorkMeta[] }) {
  /* ---------------- Featured ---------------- */
  const featured = useMemo(() => pickFeaturedThree(projects), [projects]);
  ...
  const pills = useMemo(() => {
    ...
    projects.forEach((it) =>
      getPillsForWork(it).forEach((p) => set.add(String(p)))
    );
  ...
  const focusPill = (i: number) => {
    requestAnimationFrame(() => pillRefs.current[i]?.focus());
  };
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    if (activePill !== "All") {
      list = list.filter((it) => workMatchesPill(it, activePill));
    }

    if (q) {
      list = list.filter((it) => normalize(workSearchText(it)).includes(q));
    }

    return [...list].sort((a, b) =>
      sort === "oldest"
        ? safeTime(a.date) - safeTime(b.date)
        : safeTime(b.date) - safeTime(a.date)
    );
  }, [projects, activePill, query, sort]);
  ...
  // Hide Featured when filtering OR searching
    ...
    <main>
      <Container className="py-section">
        <div className="space-y-card">
          {/* crumbs (match Email Services pattern) */}
          <Breadcrumbs className="text-text/60" items={[{ label: "Work" }]} />

          {/* page header (match Email Services pattern) */}
          <div className="space-y-tile">
            <h1 className="text-h1 tracking-tightish text-white">Work</h1>
            <p className="text-body text-muted max-w-[70ch]">
              Selected case studies and client work showcasing design systems, branding,
                ...
                >
                  ...
                  <div className="ggx-hide-scrollbar flex gap-4 px-2 snap-x snap-mandatory">
                    {featured.map((it, idx) => (
                      ...
                      >
                        <FeaturedCard item={it} href={`/work/${it.slug}`} />
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <section>
              <div className="mb-4 mt-0 flex items-end justify-between">
                ...
                <div className="relative inline-flex items-center">
                  ...
                  <svg
                    className="pointer-events-none absolute right-1 top-1/2 -translate-y-1/2 text-muted/70"
                    width="14"
                    height="14"
                    viewBox="0 0 24 24"
                    fill="none"
              ...
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 lg:gap-5">
                {visibleItems.map((it) => (
                  <GridCard key={it.slug} item={it} href={`/work/${it.slug}`} />
                ))}
              </div>
              ...
              <div className="mt-7">
                <div className="text-sm text-muted">
                  Showing{" "}
                  <span className="font-semibold text-text">{showingFrom}</span>-
...
function FeaturedCard({ item, href }: { item: WorkMeta; href: string }) {
  return (
...
function GridCard({ item, href }: { item: WorkMeta; href: string }) {
  return (
    <Link href={href} className="group block">
